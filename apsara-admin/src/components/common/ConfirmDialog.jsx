...
export function ConfirmDialog() {
  ...
  return (
    <div className='fixed inset-0 bg-[#1B4332]/20 backdrop-blur-md flex items-center
                    justify-center z-[200] p-4 animate-in fade-in duration-300'>
      <div className='bg-white rounded-[40px] shadow-2xl w-full max-w-sm overflow-hidden border border-green-50'>
        <div className='p-10 text-center'>
          <div className='w-16 h-16 bg-red-50 rounded-[24px] flex items-center justify-center
                          text-3xl mx-auto mb-6'>⚠️</div>
          <h3 className='text-xl font-black text-slate-800 mb-2'>Wait a moment</h3>
        ...
        <div className='flex gap-3 px-10 pb-10'>
          <button onClick={() => dispatch(hideConfirm())}
            className='flex-1 bg-slate-50 py-4 rounded-2xl font-black
          ...
          <button onClick={handleConfirm}
            className='flex-1 bg-red-500 hover:bg-red-600 text-white font-black
                       py-4 rounded-2xl transition shadow-xl shadow-red-900/20 text-[10px] uppercase tracking-widest'>
            Confirm
          </button>
        </div>
      </div>
